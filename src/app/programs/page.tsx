...
const getProgramProducts = async (): Promise<Product[]> => {
    ...
    if (error) {
        ...
        throw new Error("Failed to fetch program products");
    }
...
export default function ProgramsPage() {
  ...
  const { data: products, isLoading, error } = useQuery({
    queryKey: ['products', 'programs'],
    queryFn: getProgramProducts
  });
    ...
    <div className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex items-baseline gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">{t.programsPage.description}</p>
        </div>
      </div>
      {products && products.length > 0 ? (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 sm:gap-6">
         ...
         <div className="text-center py-20">
            <p className="text-muted-foreground">{t.programsPage.noProducts}</p>
        </div>
      )}
      <ScrollToTop />
    </div>
